...
(*
let rec algo_profondeur_old (g:int graph) (sommets_list: int list) (src : int) (tgt : int) : (int list) =
  ...
  if (List.exists (fun a -> a.tgt =tgt) arcs) then begin (* Si la tgt est dans les arcs *)
    ...
    if (a.lbl = 0) then
      [] (* On ne doit pas s'arrêter, il faut continuer à chercher (old : [])*)
    else 
    [src;tgt] 
    end 
  else 
      ...
      match arcs with 
        ...
        |x::rest -> 
          if ((List.mem x.tgt sommets_list) || (x.lbl =0)) then
            ...
            let chemin_suivant = algo_profondeur_old g (x.tgt :: sommets_list) x.tgt tgt in
